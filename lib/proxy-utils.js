...
export function proxyHlsRequest(xhr, url) {
  ...
  if (!url.startsWith("/api/proxy") && url.includes("://")) {
    ...
    // Set required headers for HLS requests
    xhr.setRequestHeader("Origin", "https://twoseven.xyz");
    xhr.setRequestHeader("Referer", "https://www.cineby.app/");
    xhr.setRequestHeader("x-twoseven-ext-tab-media", "1");
  }
}
